...
class TestPostList:
    def test_anonymous_user_sees_only_published_posts(
        self, api_client, published_post, draft_post
    ):
    ...
    def test_authenticated_user_sees_own_drafts_and_all_published_posts(
        self,
        api_client,
        user,
        published_post,
        draft_post,
        published_post_by_another_user,
        draft_post_by_another_user,
    ):
    ...
    def test_authenticated_user_can_create_post_with_tags(
        self, api_client, user, tag_python, tag_django
    ):
    ...
    def test_posts_are_ordered_by_ownership_then_status_then_published_at_desc(
        self,
        api_client,
        user,
        published_post,
...
class TestPostFilters:
    def test_filter_by_single_tag(
        ...
        published_post.tags.add(
            tag_python
        )  # the other post has no tags initially
    ...
    def test_filter_posts_by_multiple_tags_using_single_query_param_act_as_OR(
        self,
        api_client,
        tag_python,
        tag_django,
        ...
        url = (
            reverse('post-list') + f'?tags={tag_python.name},{tag_django.name}'
        )
        response = api_client.get(url)
    ...
    def test_filter_posts_by_multiple_tags_using_multiple_query_params_act_as_AND(
        self,
        api_client,
        tag_python,
        tag_django,
        ...
        url = (
            reverse('post-list')
            + f'?tags={tag_python.name}&tags={tag_django.name}'
        )
        response = api_client.get(url)
    ...
    def test_filter_by_author_username(
        self, api_client, user, published_post, published_post_by_another_user
    ):
    ...
    def test_filter_posts_by_invalid_published_date_returns_400(
        self, api_client
    ):
    ...
    def test_filter_posts_by_published_date_range(
        self, api_client, published_post, published_post_by_another_user
    ):
        """Test that filtering by published date range returns correct posts."""
        published_post.published_at = timezone.now() - timezone.timedelta(
            days=1
        )
        published_post.save()
        published_post_by_another_user.published_at = (
        ...
        url = (
            reverse('post-list')
            + f'?published_after={published_after.date()}&published_before={published_before.date()}'
        )
        response = api_client.get(url)
    ...
    def test_filter_posts_by_published_after_only(
        self, api_client, published_post, published_post_by_another_user
    ):
        ...
        url = (
            reverse('post-list') + f'?published_after={published_after.date()}'
        )
        response = api_client.get(url)
    ...
    def test_filter_posts_by_published_before_only(
        self, api_client, published_post, published_post_by_another_user
    ):
        ...
        url = (
            reverse('post-list')
            + f'?published_before={published_before.date()}'
        )
        response = api_client.get(url)
...
class TestPostDetail:
    def test_anonymous_user_can_read_published_post(
        self, api_client, published_post
    ):
    ...
    def test_anonymous_user_cannot_read_draft_post(
    ...
    def test_authenticated_user_can_read_own_published_post(
        self, api_client, user, published_post
    ):
    ...
    def test_authenticated_user_can_read_others_published_post(
        self, api_client, user, published_post_by_another_user
    ):
    ...
    def test_authenticated_user_can_read_own_draft_post(
        self, api_client, user, draft_post
    ):
    ...
    def test_authenticated_user_cannot_read_others_draft_post(
        self, api_client, user, draft_post_by_another_user
    ):
    ...
    def test_authenticated_user_can_delete_own_published_post(
        self, api_client, user, published_post
    ):
    ...
    def test_authenticated_user_can_delete_own_draft_post(
        self, api_client, user, draft_post
    ):
    ...
    def test_authenticated_user_cannot_delete_others_published_post(
        self, api_client, user, published_post_by_another_user
    ):
    ...
    def test_authenticated_user_cannot_delete_others_draft_post(
        self, api_client, user, draft_post_by_another_user
    ):
    ...
    def test_authenticated_user_can_update_own_post(
        self, api_client, user, published_post
    ):
    ...
    def test_authenticated_user_cannot_update_others_post(
        self, api_client, user, published_post_by_another_user
    ):
        """Test that an authenticated user cannot update another user's post."""
        api_client.force_authenticate(user=user)
        url = reverse('post-detail', args=[published_post_by_another_user.id])
        data = {'title': 'Malicious Update', 'content': 'Hacked content'}
        response = api_client.put(url, data)
...
class TestTagList:
    def test_anonymous_user_can_list_tags(
        self, api_client, tag_python, tag_django
    ):
...
class TestTagDetail:
    ...
    def test_authenticated_user_can_update_tag(
        self, api_client, user, tag_python
    ):
    ...
    def test_authenticated_user_can_delete_tag(
        self, api_client, user, tag_python
    ):
